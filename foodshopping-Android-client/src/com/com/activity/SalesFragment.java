...
import com.google.gson.reflect.TypeToken;
	...
	public class SalesFragment extends Fragment implements OnItemClickListener, IXListViewListener {
		private View mView;
...
//		public static final String BASE_URL = "http://192.168.1.193/foodshopping-servers/";
		...
		private int number;
		public SalesFragment(Handler handler,int number) {
		...
		class GetDataThread extends Thread {
			...
			public GetDataThread(int minRow,boolean isRefresh) {
				SalesFragment.this.minRow = minRow;
			...
			public void run() {
				...
				try {
					...
					if (number == 1) {
						...
						url = BaseUrl.BASE_URL + "/servlet/SalesServlet?categoryflavor_id=" + categoryflavor_id ;
					} else if (number == 2){
						url = BaseUrl.BASE_URL + "/servlet/SalesServlet2?categoryflavor_id=" + categoryflavor_id ;
					}
					String strJson = HttpUtil.getRequest(url);
//					System.out.println("strJson空？？？？？？？" + strJson);
					Gson gson = new Gson();
...
//					Collections.shuffle(cates);
		...
		class ProductAdapter extends BaseAdapter implements OnScrollListener {
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				viewHolder.textViewPrice.setText("￥"+shoppingCate.getCate_price()+ "  销量：" + shoppingCate.getCate_sale_count() );

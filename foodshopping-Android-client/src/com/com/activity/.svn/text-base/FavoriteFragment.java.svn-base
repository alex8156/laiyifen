...
import com.com.activity.FavoriteFragment.ProductAdapter.ViewHolder;
...
public class FavoriteFragment extends Fragment implements OnItemClickListener, IXListViewListener {
	private View mView;
	...
	public FavoriteFragment(Handler addBasketHandler) {
		super();
	...
	public void init() {
		// TODO Auto-generated method stub
		favoriteActivity = (FavoriteActivity) getActivity();
		executorService = Executors.newFixedThreadPool(6); //初始化线程池
		imageUtil =  new ImageUtil(favoriteActivity);
	...
	class GetDataThread extends Thread {
		...
		public GetDataThread(int minRow,boolean isRefresh) {
			FavoriteFragment.this.minRow = minRow;
		...
		public void run() {
			...
			try {
				String url = BaseUrl.BASE_URL + "/FavoriteServlet?maxRow=" + maxRow +"&minRow=" + minRow;
...
//				Collections.shuffle(cates);
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.what == RESULT) {
				...
				Toast.makeText(favoriteActivity, "加入购物车成功", Toast.LENGTH_SHORT).show();
			} else if(msg.what == ADD_TO_BASKET) {
				Toast.makeText(favoriteActivity, "服务器异常", Toast.LENGTH_SHORT).show();
			}
	...
	class ProductAdapter extends BaseAdapter implements OnScrollListener {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = LayoutInflater.from(favoriteActivity).inflate(R.layout.item_product_list, null);
						...
						public void run() {
							try {
								...
								if (user_id == 0) {
									...
									String url = BaseUrl.BASE_URL + "/AddToBasketServlet?cate_name="+shoppingCate.getCate_name()+"&cate_price="+shopping_basket_price+"&user_id="+userService.getUser_id();
			...
			viewHolderHash.put(position, viewHolder);
...
//				final String url = BaseUrl.BASE_URL + "/ProductImagePathServlet?cate_image_id="+shoppingCate.getCate_image_id();
//				new UpdateImageTask(position).execute(url);
				executorService.execute(downloadImage(position));
			...
			return new Thread() {
				...
				public void run() {
					...
					try {
							...
							imageNameHash.put(position, imageName);
							String imagesDir = PathUtil.getExternalStorageDirectory(favoriteActivity) + "/laiyifen/images/";
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		Intent intent = new Intent(favoriteActivity, ProductDetailActivity.class);
...
//	class UpdateImageTask extends AsyncTask<String, Void, Bitmap>{
//		private int position;
//		
//		public UpdateImageTask(int position) {
...
//				}
//				// 下载完以后，加载一遍，放入缓存
//				bitmap = imageUtil.loadSdcardItemImage(imagePath);
//			} catch (Exception e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			}
...
//			if (result != null) {
//				
//				viewHolderHash.get(position).imageViewPic.setImageBitmap(result);
//			}
//		};
//	}

...
import com.google.gson.reflect.TypeToken;
	...
	public class PriceFragment extends Fragment implements OnItemClickListener, IXListViewListener {
		private View mView;
...
//		public static final String BASE_URL = "http://192.168.1.193/foodshopping-servers/";
		...
		public PriceFragment(Handler handler,int number) {
		...
		class GetDataThread extends Thread {
			...
			public GetDataThread(int minRow,boolean isRefresh) {
				PriceFragment.this.minRow = minRow;
			...
			public void run() {
				...
				try {
					String url = null;
					if (number == 1) {
						url = BaseUrl.BASE_URL + "/servlet/PriceServlet?categoryflavor_id=" + categoryflavor_id ;
					} else if (number == 2){
						url = BaseUrl.BASE_URL + "/servlet/PriceServlet2?categoryflavor_id=" + categoryflavor_id ;
					}
